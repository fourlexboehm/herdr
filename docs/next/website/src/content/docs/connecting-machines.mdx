...
Relay machines let two Herdr installations connect when neither side can accept an inbound connection. The target and controller both make outbound WSS connections to a Cloudflare Worker. A hibernating Durable Object joins the live sockets, but the existing Herdr endpoint stream is encrypted end to end with Noise before it reaches the relay.

This implementation is available on Linux and macOS. It is experimental, `snow` has not received a formal security audit, and the complete design has not received an independent review. Do not use it yet for high-risk remote access. Cloudflare can observe route use, connection timing, message sizes, and IP addresses even though it cannot decrypt Herdr traffic.

### Deploy the relay

The relay project is in `cloudflare/relay/`. Cloudflare's Free plan supports the SQLite-backed Durable Objects used here, but continuous terminal output can exhaust free daily request allowances. Check Cloudflare's current pricing and limits before relying on it.
...
The Worker creates one Durable Object per random target route. It stores only a salted registration-capability verifier and bounded abuse counters. Hibernating socket attachments contain a role and connection ID. Herdr data, Noise keys, invitations, private device keys, and terminal history are not stored. Automatic invocation logs and traces are disabled because route IDs are carried in request paths.
...
- Cloudflare's hibernating WebSocket API does not expose buffered bytes or asynchronous send completion. The Durable Object keeps no userland payload queue and disconnects the failed recipient if `send()` throws; Herdr's bounded device-side queues provide the observable backpressure boundary.
