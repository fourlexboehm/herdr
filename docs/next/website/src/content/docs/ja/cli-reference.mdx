...
herdr machine add-relay --label "Studio" < relay-invitation.txt
...
`machine add` はリモートのデフォルトセッションを使います。名前付きセッションを使う場合だけ `--remote-session <name>` を追加します。`machine add-relay` は自分のコードを表示し、相手のコードを貼り付けて Enter を押すと暗号化接続を確認します。両方のデバイスで実行し、初回は `--url` を指定します。標準入力からの招待インポートも引き続き使えます。スクリプトでは `machine list` に `--json` を追加でき、各行は `ssh` または `relay` を示します。詳しくは[マシンへの接続](/ja/docs/connecting-machines/)を参照してください。
...
Relay は実験的で Linux/macOS 向けです。必要なサーバーは自動起動します。この設定に対応した実行中サーバーは、有効化・無効化・失効を再起動せずに反映します。古い Relay ビルドのサーバーは先に更新してください。`relay status --json` は保存済み設定に加えて、実行中サーバーの接続状態とコントローラー数を返します。Relay プロファイルは TUI のネイティブマシンとして動作しますが、`--machine` による JSON API 転送は現在 SSH プロファイル専用です。
