...
实验性的 Relay 机器让目标端和控制端都向 Cloudflare Worker 建立出站 WSS 连接，因此目标端不需要开放入站端口，也不需要 Cloudflare Tunnel。Durable Object 只负责连接实时套接字；现有 Herdr endpoint generation 1 字节流会先用 Noise 做端到端加密。
...
Cloudflare Free 计划支持这里使用的 SQLite-backed Durable Objects，但持续的终端输出可能耗尽每日免费请求额度。Relay 不保存 Herdr 数据、Noise 密钥、邀请或终端历史；它只保存加盐的注册校验值和有界的防滥用计数器。
