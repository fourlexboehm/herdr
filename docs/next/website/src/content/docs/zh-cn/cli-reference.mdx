...
`machine add` 默认使用远程默认会话。只有需要命名会话时才添加 `--remote-session <name>`。`machine add-relay` 显示本机配对码，等待粘贴对方的码并按 Enter，然后验证加密连接。两端均需运行，首次使用 `--url` 指定 Relay。仍支持从标准输入导入邀请。`machine list` 支持可选的 `--json` 供脚本使用，并标明每条配置的 `ssh` 或 `relay` 传输类型。完整流程见[连接机器](/zh-cn/docs/connecting-machines/)。
...
Relay 是面向 Linux/macOS 的实验性功能。需要的服务器会自动启动；支持此流程的运行中服务器无需重启即可应用启用、禁用和撤销。旧版 Relay 服务器需先更新。`relay status --json` 同时返回保存的配置、运行中服务器的连接状态和控制端数量。Relay 配置可作为 TUI 原生机器使用，但 `--machine` JSON API 转发目前仍仅支持 SSH 配置。
