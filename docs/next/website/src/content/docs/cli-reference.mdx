...
`relay enable` creates a private static device identity, random route, and independent relay-registration capability when the target has no relay state. It starts the configured server if needed and waits for relay hosting to become ready. Updated running servers apply enable, disable, and re-enable changes without restarting. `relay invite` writes a one-use 15-minute invitation to stdout and a warning to stderr. `relay devices` exposes controller labels, roles, public keys, and pairing times; `relay revoke` removes one key and active relay hosting disconnects it on the next authorization refresh.
