...
herdr machine add-relay [--url <wss-url>] [--label "Studio"]
...
By default, SSH `machine add` uses the remote default session. Add `--remote-session <name>` only for a named session. `machine add-relay` prints this device's pairing code and accepts the other device's code with Enter, then verifies the encrypted endpoint connection. Run it on both devices; `--url` is optional after initial setup. Redirected-stdin invitation imports remain supported; never put the code in a command argument. `machine list` identifies each profile's `ssh` or `relay` transport and accepts optional `--json` for scripts. See [Connecting machines](/docs/connecting-machines/) for both setup paths.
...
Workspace, tab, pane IDs, and agent names belong to a single server. Selecting a machine in the UI does not retarget CLI commands. Use the global **prefix** `--machine <label-or-id>` to route API commands to a saved SSH machine. Relay profiles currently participate in the native TUI endpoint surface but not this JSON API forwarding prefix:
...
`relay enable` creates a private static device identity, random route, and independent relay-registration capability when the target has no relay state. It starts the configured server if needed and waits for relay hosting to become ready. Peers use WSS only for setup and signaling; endpoint traffic requires direct ICE/STUN UDP or configured TURN. Updated running servers apply enable, disable, and re-enable changes without restarting. `relay invite` writes a one-use 15-minute invitation to stdout and a warning to stderr. `relay devices` exposes controller labels, roles, public keys, and pairing times; `relay revoke` removes one key and active relay hosting disconnects it on the next authorization refresh.

The feature is experimental and Linux/macOS-first. See [Encrypted relay through firewalls](/docs/connecting-machines/#encrypted-relay-through-firewalls) for deployment, pairing, security limits, and Cloudflare Free-plan behavior.
