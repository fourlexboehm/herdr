...
async fn run_controller(
    ...
    websocket
        .send(Message::Binary(secure.encrypt(&confirmation)?.into()))
        .await
        .map_err(websocket_error)?;
    let confirmation = next_binary(&mut websocket, HANDSHAKE_TIMEOUT).await?;
    let confirmation = secure.decrypt(&confirmation)?;
    if decode_secure_frame(&confirmation)? != (SecureFrameKind::ServerConfirm, &[][..]) {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "relay target did not confirm controller authorization",
        ));
    }
            ...
            outbound = outbound_rx.recv() => {
                match outbound {
                    Some(LocalEvent::Data(data)) => {
                        ...
                        sink.send(Message::Binary(secure.encrypt(&frame)?.into()))
                            .await
                            .map_err(websocket_error)?;
                    }
                ...
                match message {
                    Message::Binary(ciphertext) => {
                        ...
                        if kind != SecureFrameKind::Data {
                            return Err(super::protocol::invalid_data("unexpected secure relay control frame"));
                        }
...
impl TargetConnection {
    fn handshake_expired(&self, now: Instant) -> bool {
        match self {
            Self::AwaitingHandshake { deadline } | Self::AwaitingConfirmation { deadline, .. } => {
                *deadline <= now
            }
            ...
            _ = authorization_refresh.tick() => {
                ...
                let revoked = connections
                    ...
                    .filter_map(|(connection_id, connection)| match connection {
                        ...
                        | TargetConnection::AwaitingConfirmation { device_public_key, pairing: None, .. }
            ...
            local = local_rx.recv() => {
                ...
                match event {
                    LocalEvent::Data(data) => {
                        let Some(TargetConnection::Connected { secure, .. }) = connections.get_mut(&connection_id) else {
                            continue;
                        };
                        let frame = encode_secure_frame(SecureFrameKind::Data, &data)?;
        ...
        RelayEnvelopeKind::Data => {
            ...
            let next = match connection {
                ...
                TargetConnection::AwaitingConfirmation {
                    ...
                    let plaintext = secure.decrypt(&envelope.payload)?;
                    if decode_secure_frame(&plaintext)? != (SecureFrameKind::ClientConfirm, &[][..])
                    {
                        return Err(io::Error::new(
                            io::ErrorKind::PermissionDenied,
                            "relay controller confirmation is invalid",
                        ));
                    }
                    if let Some(pairing) = pairing {
                        RelayHostState::update(|current| {
                            let mut current = current.ok_or("relay host state is missing")?;
                            if !current.enabled || !current.same_host_configuration(state) {
                                return Err(
                                    "relay host configuration changed during pairing".into()
                                );
                            }
                            current.complete_pairing(
                    ...
                    if !current.enabled
                        || !current.same_host_configuration(state)
                        || current.paired_device(&device_public_key).is_none()
                    {
                        return Err(io::Error::new(
                            io::ErrorKind::PermissionDenied,
                    ...
                    let local = connect_target_local(
                        client_socket_path,
                        envelope.connection_id,
                        local_tx.clone(),
                    )?;
                    let confirmation = encode_secure_frame(SecureFrameKind::ServerConfirm, &[])?;
                    send_target_data(sink, envelope.connection_id, secure.encrypt(&confirmation)?)
                        .await?;
                ...
                TargetConnection::Connected {
                    ...
                    if kind != SecureFrameKind::Data {
                        return Err(super::protocol::invalid_data(
                            "unexpected secure relay control frame",
                        ));
                    }
                    local
                        .inbound
